...
const CourseList: React.FC = () => {
  ...
  const [selectedLearningMethod, setSelectedLearningMethod] =
    useState<string>("");
  const [selectedStatus, setSelectedStatus] = useState<string>("");

  const sortedCourses = [...courses].sort((a, b) => (b.date > a.date ? 1 : b.date < a.date ? -1 : 0));
  ...
  const filteredCourses = sortedCourses.filter((course) => {
    ...
    const matchesLearningMethod =
      ...
      course.learningMethod === selectedLearningMethod;
    const matchesStatus =
      !selectedStatus ||
    ...
    return (
      matchesSearch && matchesTopic && matchesLearningMethod && matchesStatus
    );
  });
            ...
            <div className="flex bg-white rounded-lg shadow-sm p-4 gap-6 hover:shadow-md transition">
              ...
              <div className="flex-1 flex flex-col justify-between">
                ...
                <div>
                  ...
                  <span
                    className={`px-2 py-1 border text-xs rounded-md font-medium transition-colors ${course.statusColor}`}
                  >
                    {course.status}
                  </span>
                </div>
